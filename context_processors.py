...
def get_table_owner_status():
    ...
    if user_id:
        ...
        cursor.execute("SELECT table_owner, times_logged FROM users WHERE user_id = ?", (user_id,))
        row = cursor.fetchone()
        conn.close()
    ...
    return dict(user_id=user_id, is_table_owner=is_table_owner, table_owner_title=table_owner_title, times_logged=times_logged)
